...
{{/*
Maps a Service port name (as used in ingress.hosts[].paths[].portName and
httpRoute.matches[].portName) to its numeric port, for use where a port
number is required (e.g. Gateway API HTTPRoute backendRefs).
*/}}
{{- define "frigate.portNumber" -}}
{{- $portMap := dict "http" .service.port "http-auth" 8971 "rtmp" 1935 "rtsp" 8554 "webrtc-tcp" 8555 "webrtc-udp" 8555 "go2rtc-admin" 1984 -}}
{{- index $portMap .name -}}
{{- end -}}
